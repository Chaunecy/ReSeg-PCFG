...
def parse_command_line(program_info):
    ...
    parser.add_argument(
        '--ngram',
        '-n',
        help='<ADVANCED> The depth to generate conditional probabilities ' +
...
def main():
    ...
    pcfg_parser.leet_detector.init_l33t(program_info['training_file'], program_info['encoding'])
    # Loop until we hit the end of the file
    try:
        password = file_input.read_password()
        while password:
    ...
    # Save the OMEN data
    if not save_omen_rules_to_disk(omen_trainer, omen_keyspace, omen_levels_count, num_valid_passwords, base_directory,
                                   program_info):

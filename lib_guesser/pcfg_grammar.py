...
class PcfgGrammar:
    ...
    #
    def __init__(self, rule_name, base_directory, version, save_file=None, skip_brute=False, skip_case=False,
                 debug=False, base_structure_folder="Grammar", guess_number=0, save_guesses_to_file=None):

        # Debugging and Status Information
        #
        self.guesses_generate_done = False
        ...
        self.grammar, self.base, self.ruleset_info = load_grammar(rule_name, base_directory, version, skip_brute,
                                                                  skip_case, base_structure_folder)
        ...
        if save_guesses_to_file is not None:
            self.save_to_file(save_guesses_to_file)
    ...
    def print_guess(self, guess):
        if not self.debug:
            try:
                if self._cur_guesses > self.guess_number:
                    sys.exit(0)
                self._cur_guesses += 1
                print(guess)
    ...
    def write_guess_to_file(self, guess):
        if self.guesses_generate_done:
            return
        elif self._cur_guesses >= self.guess_number:
            self.shutdown()
            self.guesses_generate_done = True
        else:
            self._cur_guesses += 1
            self.output_file.write("{}\n".format(guess))
            if self._cur_guesses % 10000 == 0:

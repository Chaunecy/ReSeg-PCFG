...
def load_grammar(grammar, rule_directory):
    ...
    try:
        ...
        #
        # Note, not going off the generic grammar since that doesn't contains
        # info that we will need for essentially 're-stemming' the input words
        # to score
        #
        ...
        if not _load_from_multiple_files(grammar.count_alpha_masks, config['CAPITALIZATION'], rule_directory,
                                         grammar.encoding):
            return False
